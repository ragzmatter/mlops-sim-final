...
echo "==> 3b) Redis online store + Feast materialization"
kubectl apply -f feast/redis-and-materialize-job.yaml
kubectl rollout status deployment/redis -n mlops-data --timeout=60s
echo "    (feast materialize now runs as a KFP component, not a standalone Job here)"
...
echo "==> 8) Retraining — now a KFP Recurring Run, not a standalone CronJob"
echo "    Run once: python3 kubeflow/pipeline.py && python3 kubeflow/recurring_run.py"
echo "    (skipping kubectl apply here — see _deprecated/README.md for why)"
